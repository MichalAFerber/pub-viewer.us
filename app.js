(function(){
  ...
  function show(data, name){
    currentName = name || ""; rawText = ""; parseErr = "";
    ...
    dataView.innerHTML = ""; dataView.className = "dataview pub";
    var bytes = new Uint8Array(data);
    try { pubData = PUBCORE.parse(bytes); }
    catch (e){ pubNotice("Not a Publisher file", "This doesn’t look like a Microsoft Publisher (.pub) or other OLE compound file. (" + (e && e.message || e) + ")"); return; }
    hasRendered = true;
    renderPub(pubData);
    if (typeof revealHeader === "function") revealHeader();
  }

  function pubKV(label, value){
    if (value == null || value === "") return null;
    if (value instanceof Date) value = value.toISOString().replace("T", " ").replace(/\.\d+Z$/, " UTC");
  ...
  function pubSection(title){ var s = el("div", "pub-sec"); var h = el("div", "pub-sec-h"); h.textContent = title; s.appendChild(h); return s; }

  function renderPub(p){
    dataView.innerHTML = "";
    var banner = el("div", "pub-banner");
    var bt = el("div", "pub-banner-t"); bt.textContent = "Best-effort preview";
    var bp = el("p");
    bp.textContent = "Microsoft Publisher’s format is proprietary and undocumented, so the page layout, images and fonts can’t be reconstructed in the browser. Shown below is the document’s metadata and the readable text extracted from it. For a faithful copy, open it in Publisher and use “Save As → PDF” (then read it in pdf-viewer.us).";
    banner.appendChild(bt); banner.appendChild(bp);
    dataView.appendChild(banner);

    var m = p.meta, d = p.dsi;
    var info = pubSection("Document info");
    var rows = [["Title", m.Title], ["Author", m.Author], ["Subject", m.Subject], ["Keywords", m.Keywords],
      ["Comments", m.Comments], ["Last saved by", m.LastSavedBy], ["Application", m.AppName],
      ["Company", d.Company], ["Manager", d.Manager], ["Category", d.Category],
      ["Created", m.Created], ["Last saved", m.LastSaved],
      ["Pages", m.PageCount], ["Words", m.WordCount], ["Characters", m.CharCount]];
    var any = false;
    for (var i = 0; i < rows.length; i++){ var r = pubKV(rows[i][0], rows[i][1]); if (r){ info.appendChild(r); any = true; } }
    if (!any){ var none = el("div", "cv-row"); none.textContent = "No document metadata found."; info.appendChild(none); }
    var meta = el("div", "cv-row"); // container size line
    var sz = el("span", "cv-k"); sz.textContent = "File"; var szv = el("span", "cv-v");
    szv.textContent = "OLE compound file · " + Math.round(p.size / 1024) + " KB"; meta.appendChild(sz); meta.appendChild(szv);
    info.appendChild(meta);
    dataView.appendChild(info);

    if (p.text && p.text.length){
      var ts = pubSection("Extracted text (" + p.text.length + " fragment" + (p.text.length === 1 ? "" : "s") + ")");
      var box = el("div", "pub-text");
      for (var t = 0; t < p.text.length; t++){ var para = el("p", "pub-line"); para.textContent = p.text[t]; box.appendChild(para); }
      ts.appendChild(box); dataView.appendChild(ts);
    } else {
      var nt = pubSection("Extracted text");
      var np = el("p", "pub-none"); np.textContent = "No readable text could be recovered from this file."; nt.appendChild(np);
      dataView.appendChild(nt);
    }
  }
  function pubNotice(title, msg){
    dataView.innerHTML = ""; dataView.className = "dataview pub";
    var box = el("div", "pub-notice");
    var h = el("div", "pub-notice-title"); h.textContent = title;
    var p = el("p"); p.textContent = msg;
    var q = el("p", "pub-notice-sub"); q.textContent = "Your file was not uploaded anywhere.";
    box.appendChild(h); box.appendChild(p); box.appendChild(q); dataView.appendChild(box);
  }
  function buildRendered(){ if (pubData) renderPub(pubData); }
  function readFile(file){
    if (!file) return;
    if (!isAccepted(file.name)){ if (!familyRoute(file)) toast("“" + file.name + "” isn’t a supported Publisher/OLE file"); return; }
  ...
  doc.addEventListener("keydown", function(e){
    if (!id("routeCard").hidden){                       // §6.10: the offer card is modal
      ...
      if (e.key === "Tab"){
        ...
        (document.activeElement === go || go.disabled ? no : go).focus();
      }
      return;
    }
  ...
  // (fv-ready → fv-file → fv-ack). Files are accepted from family origins only.
  ...
  if (fvh){
    ...
    if (window.opener){
      ...
      // While the hand-off is pending, say so in the empty-state sub-line
      // (textContent only — fvhName is untrusted); revert after ~10 s.
      var fvhSub = doc.querySelector(".empty-sub");
      if (fvhSub){
        var fvhSubWas = fvhSub.textContent;
        fvhSub.textContent = "Receiving “⁨" + fvhName + "⁩”…";
        setTimeout(function(){ fvhSub.textContent = fvhSubWas; }, 10000);
      }
    }
  }
